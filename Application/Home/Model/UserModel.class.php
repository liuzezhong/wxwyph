...
class UserModel extends \Think\Model
{
    ...
    public function listWxUser($condition = array(), $page = 1, $pageSize = 10) {
        $offset = ($page -1) * $pageSize;
        $condition['nickname'] = array('neq','');
        $res =  $this->_db->where($condition)->order('gmt_create desc')->limit($offset,$pageSize)->select();
        return $res;
    }

    public function getCountWxuser($condition = array()) {
        $condition['nickname'] = array('neq','');
        return $this->_db->where($condition)->count();
    }

    public function updateWxuserByID($user_id = 0, $data = array()) {
        if(!$user_id) {
            throw_exception('UserModel updateWxuserByID user_id is null');
        }
        if(!$data) {
            throw_exception('UserModel updateWxuserByID data is null');
        }
        $condition['user_id'] = $user_id;

        $res = $this->_db->where($condition)->save($data);
        return $res;
    }
