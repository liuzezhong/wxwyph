...
class RepaymentsModel extends Model
{
     ...
     * @return mixed
     */
    public function getRepaymentsMaxTimeByLoanID($loan_id = 0) {
        if(!$loan_id) {
            throw_exception('Wxapp Model RepaymentsModel getRepaymentsMaxTimeByLoanID loan_id is null');
        }
        $condition['loan_id'] = $loan_id;
        return $this->where($condition)->max('gmt_repay');
    }
    ...
    public function listRepayByLoanID($loan_id = 0) {
        ...
        $condition['loan_id'] = $loan_id;
        $condition['is_delete'] = array('neq',1);
        return $this->where($condition)->order('cycles desc')->select();
    }
